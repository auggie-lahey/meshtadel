...
export function buildCommitteeEvent(opts: {
  ...
  if (opts.meetingSchedule)
    tags.push(["meetingSchedule", opts.meetingSchedule]);
  if (opts.openings !== undefined)
    tags.push(["openings", String(opts.openings)]);
  if (opts.topicTags) {
...
export async function publishCommittee(
...
export async function publishCommitteeMember(
  signedEvent: Record<string, unknown>,
): Promise<boolean> {
  try {
    const responses = await pool.publish(nostrRelays, signedEvent as any);
...
export async function publishCommitteeOpening(
  signedEvent: Record<string, unknown>,
): Promise<boolean> {
  try {
    const responses = await pool.publish(nostrRelays, signedEvent as any);
    ...
    pool
      ...
      .subscribe({
        ...
        complete: () => {
          ...
          const committees: Committee[] = deduped
            .map(parseCommitteeEvent)
...
function parseCommitteeEvent(event: any): Committee | null {
  ...
  const meetingSchedule = event.tags?.find(
    (t: string[]) => t[0] === "meetingSchedule",
  )?.[1];
  const openingsStr = event.tags?.find(
    (t: string[]) => t[0] === "openings",
  )?.[1];
...
function parseCommitteeMemberEvent(event: any): CommitteeMember | null {
  ...
  const roleRaw =
    event.tags?.find((t: string[]) => t[0] === "role")?.[1] || "member";
  const role = roleRaw;
