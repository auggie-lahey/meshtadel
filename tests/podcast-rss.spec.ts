...
test.describe("@education podcast RSS rendering", () => {
  test("podcast pin with RSS feed URL renders cover image and website link", async ({
    page,
  }) => {
    ...
    await addBtn.click();
    await page.waitForSelector('[data-testid="add-pin-modal"]', {
      timeout: 5000,
    });
    ...
    await page
      .locator('[data-testid="pin-url"]')
      .fill("https://bowlafterbowl.com/feed.xml");
    await page
      .locator('[data-testid="pin-title"]')
      .fill("Bowl After Bowl (Test)");
    await page
      .locator('[data-testid="pin-description"]')
      .fill("Test podcast with RSS feed URL");
