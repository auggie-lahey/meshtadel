 ...
 */
async function waitForPinToAppear(
  page: Page,
  uniqueTitle: string,
  maxAttempts = 3,
) {
...
test.describe("Education Page - Add Resources via UI @education @whitelist", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page
      .locator('[data-testid="add-pin-btn"], [data-testid^="pin-"]')
      .first()
      .waitFor({ timeout: 15000 });
  });
  ...
  test("add a YouTube resource and verify k=web tag", async ({ page }) => {
    ...
    await page
      .getByTestId("pin-description")
      .fill("Test YouTube video added via Playwright");
  ...
  test("add a Vimeo video resource and verify k=web tag", async ({ page }) => {
    ...
    await page
      .getByTestId("pin-description")
      .fill("Test Vimeo video added via Playwright");
    ...
    const iframe = newPin.locator(
      "iframe[src*='player.vimeo.com/video/347119399']",
    );
    await expect(iframe).toBeVisible();
  ...
  test("add a Rumble video resource and verify k=web tag", async ({ page }) => {
    ...
    await page
      .getByTestId("pin-url")
      .fill("https://rumble.com/v2ea91a-the-masters-has-arrived-unpaved.html");
    await expect(page.getByTestId("detected-type")).toContainText("Videos");
    await page
      .getByTestId("pin-description")
      .fill("Test Rumble video added via Playwright");
  ...
  test("add a Podcast resource and verify k=web tag", async ({ page }) => {
    ...
    await page
      .getByTestId("pin-url")
      .fill("https://open.spotify.com/show/030JakQMatQTMOIkwVx2cQ");
    await expect(page.getByTestId("detected-type")).toContainText("Podcasts");
    await page
      .getByTestId("pin-description")
      .fill("Test podcast added via Playwright");
  ...
  test("add a Link resource and verify k=web tag", async ({ page }) => {
    ...
    await page
      .getByTestId("pin-description")
      .fill("Test link added via Playwright");
  ...
  test("add a Book resource via ISBN and verify k=isbn tag", async ({
    page,
  }) => {
    ...
    const coverImg = newPin.locator(
      "img[src*='pictures.abebooks.com/isbn/9780743273565']",
    );
    await expect(coverImg).toBeVisible();

    // Verify link goes to bookfinder.com
    const bookLink = newPin.locator(
      "a[href*='bookfinder.com/isbn/9780743273565']",
    );
  ...
  test("add a Paper resource via DOI and verify k=doi tag", async ({
    page,
  }) => {
    ...
    const badge = newPin
      .locator("span.inline-flex")
      .filter({ hasText: "Papers" });
    await expect(badge).toBeVisible();

    // Verify link goes to doi.org
    const paperLink = newPin.locator(
      "a[href='https://doi.org/10.1038/171737a0']",
    );
  ...
  test("add a Location resource via coordinates and verify k=geo tag", async ({
    page,
  }) => {
    ...
    await page
      .getByTestId("pin-description")
      .fill("Test location via coordinates");
  ...
  test("add a Podcast Episode resource and verify Spotify player embed", async ({
    page,
  }) => {
    ...
    await page
      .getByTestId("pin-url")
      .fill("https://open.spotify.com/episode/1WKigLfNJ1X09srlcWNgmy");
    // Verify detected type shows Episodes
    await expect(page.getByTestId("detected-type")).toContainText("Episodes");
    await page
      .getByTestId("pin-description")
      .fill("Test podcast episode added via Playwright");
    ...
    const iframe = newPin.locator(
      "iframe[src*='open.spotify.com/embed/episode/1WKigLfNJ1X09srlcWNgmy']",
    );
    await expect(iframe).toBeVisible();
    ...
    expect(rawText).toContain(
      "open.spotify.com/episode/1WKigLfNJ1X09srlcWNgmy",
    );
  });
  ...
  test("add an Article resource and verify k=article tag", async ({ page }) => {
    ...
    await page
      .getByTestId("pin-summary")
      .fill("A test article description for Playwright");
    await page
      .getByTestId("pin-description")
      .fill(
