...
async function publishPin(
  ...
  await expect(page.getByTestId("add-pin-modal"))
...
test.describe("Education Page @education", () => {
  ...
  test("displays page header", async ({ page }) => {
    await expect(
      page.getByRole("heading", { name: /Education Resources/i }),
    ).toBeVisible();
  });
  ...
  test("featured tab is active by default", async ({ page }) => {
    await expect(page.getByTestId("tab-featured")).toHaveClass(
      /bg-bitcoin-orange/,
    );
  });
});
...
test.describe("Education Page - With Data @education @whitelist", () => {
  ...
  test("filter to Videos shows only video pins with embeds", async ({
    page,
  }) => {
    const pins = page.locator('[data-testid^="pin-"]');
    await expect(pins.first()).toBeVisible({ timeout: 15000 });
    ...
    for (let i = 0; i < Math.min(count, 3); i++) {
      ...
      expect(src).toMatch(
        /youtube\.com\/embed|player\.vimeo\.com|rumble\.com\/embed/,
      );
    }
  ...
  test("filter to Podcasts shows only podcast pins with Spotify embeds", async ({
    page,
  }) => {
    const pins = page.locator('[data-testid^="pin-"]');
    await expect(pins.first()).toBeVisible({ timeout: 15000 });
  ...
  test("filter to Links shows only generic link pins", async ({ page }) => {
    ...
    const mediaIframes = page.locator(
      'iframe[src*="youtube.com"], iframe[src*="spotify.com"]',
    );
  ...
  test("EventActions dropdown has Share, View Raw Data, Copy options", async ({
    page,
  }) => {
    const pins = page.locator('[data-testid^="pin-"]');
    await expect(pins.first()).toBeVisible({ timeout: 15000 });
  ...
  test("switches to pinboards tab", async ({ page }) => {
    ...
    await expect(page.getByTestId("tab-boards")).toHaveClass(
      /bg-bitcoin-orange/,
    );
  });
